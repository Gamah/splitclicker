...
public static class ApiClient
{
	...
	/// (/api/{ver}/… and /ws/{ver}). "v7" is the current build (arming-phase cursors +
	/// arming-AFK, the `touch` signal, and park/unpark deferral, on top of v6's park /
	/// Pause); set it to "v6"/"v5"/… to exercise an older or the legacy/troll path the
	/// server gives clients below its configured live version. An EMPTY string means no
	/// segment at all — raw /ws — for the legacy/unversioned old master. Mutable so the
	/// scene's ClickController can override it for testing. NOTE: the runtime value is set
	/// from ClickController.ApiVersion at startup; this default is only a fallback.</summary>
	public static string ApiVersion { get; set; } = "v7";
